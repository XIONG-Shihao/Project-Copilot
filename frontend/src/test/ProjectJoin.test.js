...
jest.mock('../services/project.service', () => ({
  getProjectDetailsFromInvite: (...args) => {
    if (!mockGetProjectDetailsFromInvite)
...
describe('ProjectJoin Component', () => {
  ...
  test('renders project details after successful fetch', async () => {
    ...
    // Wait for the join button to appear (effect & setState settled)
    await screen.findByRole('button', { name: /join project/i });
    ...
    expect(mockGetProjectDetailsFromInvite).toHaveBeenCalledWith(
      'test-invite-token'
    );
  });
  ...
  test('handles successful project join', async () => {
    ...
    await waitFor(() => {
      expect(mockJoinProjectViaInvite).toHaveBeenCalledWith(
        'test-invite-token'
      );
    });
  ...
  test('handles authentication error when fetching project details', async () => {
    ...
    expect(
      screen.getByText('You need to be logged in to join a project.')
    ).toBeInTheDocument();
    expect(
  ...
  test('handles general error when fetching project details', async () => {
    ...
    expect(
      screen.getByText('Failed to load project details. Please try again.')
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /back to dashboard/i })
    ).toBeInTheDocument();
